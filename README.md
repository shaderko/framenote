...
**Internet** mode uses the configured lightweight relay to tunnel HTTP control requests and bounded media byte ranges to the host. Before a local or internet session opens, FrameNote uses FFmpeg to prepare a 720p-or-smaller H.264/AAC playback rendition capped near 3.5 Mbps. That rendition is cached for seven days and reused while the source is unchanged; the original video is never modified. This avoids sending unnecessarily high-bitrate source bytes and prevents the audio-only/black-video failure caused by a guest webview not supporting the source video codec. The relay stores neither the project nor the rendition. Each guest exposes a standards-compliant local range server to its webview while fetching fixed 512 KiB pieces behind the scenes. While the webview consumes one piece, FrameNote prefetches up to the next eight pieces (4 MiB) in the background. Pieces are cached on disk for the session, so repeated playback and nearby seeks do not resend the same bytes.
...
Local playback support follows the operating system webview. MP4/H.264 is the safest cross-platform choice; a selected container or codec can still be rejected by the platform even when FFmpeg can analyze it. Shared sessions use FrameNote's cached H.264/AAC compatibility rendition, so guests do not depend on the original codec. FrameNote never alters the source file.
